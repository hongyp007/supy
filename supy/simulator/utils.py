...
def get_testdata_path(file):
	return f"{REFDATA_DIR}/testdata/{file}"
#----------------------------------------------------------------
def get_testdata(target):
	...
	elif target=="AT2017gfo_2.4d":
		sptbl = Table.read(get_testdata_path('AT2017gfo_2.4d.dat'), names=['lam', 'flam', 'tmp', 'tmp2'], format='ascii')
		sptbl['lam'].unit = u.Angstrom
		sptbl['flam'].unit = u.erg/((u.cm**2)*u.second*u.Angstrom)
		sptbl["fnu"] = convert_flam2fnu(sptbl['flam'], sptbl['lam'])
		sptbl["fnu"] = sptbl["fnu"].to(u.uJy)
		sptbl.remove_columns(['tmp', 'tmp2'])
	else:
		print("The testdata does not exist.")
	return sptbl
#----------------------------------------------------------------
def plot_data(data, ax=None, flux_unit="AB", **kwargs):
	...
	if flux_unit == "AB":
		...
		spflam = data["flam"]
		x = splam
		y = convert_flam2AB(spflam, splam)
	elif flux_unit == "fnu":
		x = data["lam"]
		...
		if "flam" not in data.keys():
			...
			y = data["flam"]
	ax.plot(x, y, color=kwargs.pop("c", "gray"), zorder=kwargs.pop("zorder", 0), **kwargs)
	if flux_unit == "AB":
		flux_unit = "mag"
		ax.set_ylim([y.max()+0.25, y.min()-0.25])
...
def convert_abs2app(M, d):
	...
	return m
#----------------------------------------------------------------
def convert_flam2AB(flam, lam):
	# Given spectral flux density in erg/(s cm² Å)
	flux_lambda = flam.to(u.erg / (u.s * u.cm**2 * u.AA))

	# Observing wavelength (e.g., 5000 Å)
	wavelength = lam.to(u.AA)

	# Convert to frequency-based flux density
	flux_nu = flux_lambda * wavelength**2 / const.c

	# Convert to AB magnitude
	m = -2.5 * np.log10(flux_nu.to(u.erg / (u.s * u.cm**2 * u.Hz)).value) - 48.60
	return m
#----------------------------------------------------------------
...
def apply_redshift_on_spectrum(spflam, splam, Ld = None, z=None, z0=0, scale=True):
	if Ld is None and z is not None:
		Ld = cosmo.luminosity_distance(z)
	elif Ld is not None and z is None:
		z = z_at_value(cosmo.angular_diameter_distance, Ld, method='bounded', bracket=(0.001, 10))
	else:
		return (spflam, splam)
	#	Shifted wavelength
	...
	if scale:
		...
		zspappmag = convert_abs2app(zspabsmag.value, Ld.to(u.pc).value)*u.ABmag
...
def register_custom_filters_on_speclite():
	...
	import speclite.filters
	#	Medium-band (25nm)
	rsptbl = ascii.read(f'{REFDATA_DIR}/7dt/7dt_filter.m4000_to_m8750.25nm.response.dat')
	filterlist = np.unique(rsptbl['name'])
	for filte in filterlist:
		#	Filter Table
		fltbl = rsptbl[rsptbl['name']==filte]
	...
	#	Medium-band (50nm)
	rsptbl = ascii.read(f'{REFDATA_DIR}/7dt/7dt_filter.m3750_to_m9000.50nm.response.dat')
	filterlist = np.unique(rsptbl['name'])
	for filte in filterlist:
		#	Filter Table
		fltbl = rsptbl[rsptbl['name']==filte]
	...
	#	Broad-band (SDSS)
	rsptbl = ascii.read(f'{REFDATA_DIR}/7dt/7dt_filter.ugriz.broad.response.dat')
	filterlist = np.unique(rsptbl['name'])
	for filte in filterlist:
		#	Filter Table
		fltbl = rsptbl[rsptbl['name']==filte]
...
def sensitivity_plot(spherex=True, smss=True, ps1=True, ax=None):
    ...
    #	SPHEREx
    if spherex:
        sphxcsvlist = sorted(glob.glob(f"{REFDATA_DIR}/SPHEREx/SPHEREx*.csv"))
        total_table_list = []

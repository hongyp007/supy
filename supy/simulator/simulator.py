...
class Simulator(Filter):
	...
	def __init__(self, filterset="default", verbose=True):
	...
	def _get_depth_table(self, Nsigma=5):
		...
		for ii, (cwl, filtername) in enumerate(zip(self.bandcenter, self.filterNameList)):
			...
			# photon rate
			photon_rate = utils.synth_phot(wave, flux, wave_sys, resp_sys, return_photonrate=True)
			# SB
			SB_sky = utils.synth_phot(wave, flux, self.lam, resp_lvf)
	...
	def get_synphot(self, data, splam=None, Ld=None, z=None, z0=0, show_plot=False, flux_unit="AB"):
		...
		if z is not None or Ld is not None: 
			(spflam, splam) = utils.apply_redshift_on_spectrum(spflam, splam, Ld=Ld, z=z, z0=z0)
			spflam, splam = self.speclite_bands.pad_spectrum(spflam, splam)
		...
		if show_plot:
			ax = plt.gca()
			self.plot_syn_spectrum(ax=ax, flux_unit=flux_unit, add_data=True)
	...
	def plot_syn_spectrum(self, data=None, ax = None, flux_unit="AB", add_data=False):
		...
		if add_data:
			...
			utils.plot_data(self.model_data, ax=ax, flux_unit=flux_unit, label="Input data")
	...
	def _calculate_pointsource_snr(self, mag, filtername, exposure=None):
		...
		photon_rate_src = utils.synth_phot(wave, flux_src, wave_sys, resp_sys, return_photonrate=True)  # [ph/s/cm2]
		photon_rate_sky = utils.synth_phot(wave, flux_sky, wave_sys, resp_sys, return_photonrate=True)  # [ph/s/cm2/arcsec2]
